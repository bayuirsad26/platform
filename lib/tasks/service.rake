namespace :service do
  ...
  task :influxdb, [:command] do |task, args|
    ...
    def start
      ...
      sh 'docker-compose exec -T influxdb bash -c "cat exchange.sql | influx"'
    end
  ...
  desc 'Run daemons (rango, exchange daemons, account sidekiq)'
  task :daemons, [:command] do |_task, args|
    @daemons = %w[rango blockchain cron_job upstream deposit deposit_coin_address withdraw_coin influx_writer account_sidekiq order_processor matching]
  ...
  desc 'Run setup hooks for exchange, account'
  task :setup, [:command] => ['vault:setup', 'vault:load_policies', 'render:config', 'kaisave:fetch', 'kaisave:save'] do |task, args|
    if args.command != 'stop'
      ...
      puts '----- Running hooks -----'
      sh 'docker-compose run --rm exchange bash -c "./bin/link_config && kaigara bundle exec rake db:create db:migrate"'
      sh 'docker-compose run --rm exchange bash -c "./bin/link_config && kaigara bundle exec rake db:seed"'
      sh 'docker-compose run --rm account bash -c "./bin/init_config && kaigara bundle exec rake db:create db:migrate"'
      sh 'docker-compose run --rm account bash -c "./bin/link_config && kaigara bundle exec rake db:seed"'
    end
  end

  desc 'Run mikro app (account, exchange)'
  task :app, [:command] => [:backend, :setup] do |task, args|
    args.with_defaults(:command => 'start')

    def start
      puts '----- Starting app -----'
      sh 'docker-compose up -d exchange account gateway'
    end

    def stop
      puts '----- Stopping app -----'
      sh 'docker-compose rm -fs exchange account gateway'
    end
